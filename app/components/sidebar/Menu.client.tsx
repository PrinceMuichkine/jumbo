...
export function Menu() {
  ...
  return (
    <motion.div
      ...
      className="flex flex-col side-menu fixed top-0 w-[350px] h-full bg-jumbo-elements-background-depth-2 border-r rounded-r-3xl border-jumbo-elements-borderColor z-sidebar shadow-xl shadow-jumbo-elements-sidebar-dropdownShadow text-sm"
    >
          ...
          <DialogRoot open={dialogContent !== null}>
            {binDates(list).map(({ category, items }) => (
              <div key={category} className="mt-4 first:mt-0 space-y-1">
                <div className="text-jumbo-elements-textTertiary sticky top-0 z-1 bg-jumbo-elements-background-depth-2 pl-2 pt-2 pb-1">
                  {category}
                </div>
                {items.map((item) => (
            ...
            <Dialog onBackdrop={closeDialog} onClose={closeDialog}>
              {dialogContent?.type === 'delete' && (
                <>
                  ...
                  <div className="px-5 pb-4 bg-jumbo-elements-background-depth-2 flex gap-2 justify-end">
